...
// PadRight right-pads `s` with spaces so the result occupies exactly
// `n` visible terminal cells. Delegates to tui.PadRight so wizard
// pickers stay aligned when rows carry emoji or other wide runes — a
// rune-count pad under-fills the column by one cell per wide rune and
// shifts neighbouring columns right (the bug that misaligned
// `thicket edit`'s workspace picker once nicknames carried emoji).
func PadRight(s string, n int) string {
	return tui.PadRight(s, n)
}

// Truncate caps `s` at `n` visible terminal cells, appending `…` when
// truncation actually happens. Delegates to tui.Truncate so the
// width budget matches PadRight — using a rune-count truncate would
// leave wide-rune strings overflowing the column PadRight just sized.
